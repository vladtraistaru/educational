...
On the menu the recipes stay mysterious: a grid of empty slots plus a one-line clue. The name is revealed on the results screen.
...
- Question `A × B = ?` with four tappable answer bubbles (one correct, three plausible distractors), factors 1-12.
...
- After ~900ms of feedback (4500ms for a discovery) the next question loads. At 0 lives the round ends.

### Results (`ResultScreen`)
- The crafting table as far as it got, the revealed recipe name, score, total correct, best streak.
...
- `GameScreen.tsx`: `question`, `score`, `streak`, `bestStreak`, `correctCount`, `lives`, `feedback { chosenIndex, wasCorrect }`, `mood`, `discovery { fromStageIndex, toStageIndex }`
